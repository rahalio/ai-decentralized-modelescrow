...
Package scope: `@modelescrow` (from `package_scope` in config).

**HARD RULE:** Never commit or push `.codegen/` — it is gitignored local tooling. See `.cursor/rules/no-codegen-commit.mdc`.
...
2. Keep `package_scope` `@modelescrow` and `@modelescrow/*` packages.
