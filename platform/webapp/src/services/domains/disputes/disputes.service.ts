...
const rawDisputesService = {
  ...
  async getDispute(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    ...
    const url = `/v1/disputes` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   * Get dispute
   */
  async getDispute(disputeId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/v1/disputes/${disputeId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getDispute(disputeId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }
